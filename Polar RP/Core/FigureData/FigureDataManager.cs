using Polar.Core.FigureData.Types;
using System.Drawing;
using System.Text.Json.Nodes;
using System.Xml;

namespace Polar.Core.FigureData
{
    public class FigureDataManager
    {
        ...
        public void Init()
        {
            ...
            string jsonPath = PolarEnvironment.PatchDir + "Settings/FigureData.json";
                        ...
                        foreach (var paletteNode in palettesNode)
                        {
                            ...
                            if (paletteNode["colors"] is JsonArray colArr)
                            {
                                foreach (var col in colArr)
                                {
                                    ...
                                    if (id != 0)
                                    {
                                        this._palettes[paletteId].Colors[id] = new Polar.Core.FigureData.Types.Color(id, index, club, selectable, value);
                                    }
                                }
                            }
                        }
            ...
            if (!loadedFromJson)
            {
                string xmlPath = PolarEnvironment.PatchDir + "Settings/figuredata.xml";
                    ...
                    foreach (XmlNode Child in Node.ChildNodes)
                    {
                        ...
                        foreach (XmlNode Sub in Child.ChildNodes)
                        {
                            this._palettes[Convert.ToInt32(Child.Attributes["id"].Value)].Colors.Add(
                                ...
                                new Polar.Core.FigureData.Types.Color(Convert.ToInt32(Sub.Attributes["id"].Value),

...
{
    class WiredEffectUsersWithVariable : WiredEffectVariableSelectorBase
    {
        public WiredEffectUsersWithVariable(Room instance, Item item) : base(instance, item) { }
        public override WiredBoxType Type => WiredBoxType.SelectorUsersWithVariable;
        ...
        public override bool Execute(params object[] @params)
        {
            ...
            foreach (var user in Instance.GetRoomUserManager().GetRoomUsers())
            {
                ...
                if (_selectByValue)
                {
                    int? val = WiredVariableResolver.ResolveValue(Instance, _variableToken, habbo, null, context);
                    if (!MatchesComparison(val, referenceValue)) continue;
                }
                matched.Add(habbo);
            }

            context.SelectedUsers = matched;
            return true;
        }
    }
}

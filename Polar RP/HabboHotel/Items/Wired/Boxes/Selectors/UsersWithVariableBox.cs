...
{
    /// <summary>
    /// Selects users that have a given variable (or a variable matching a comparison).
    /// Mirrors WiredEffectUsersWithVariable → WiredEffectVariableSelectorBase (TARGET_USER).
    /// Logic identical to FurniWithVariableBox; only WiredBoxType and target type differ.
    /// </summary>
    class UsersWithVariableBox : IWiredItem, IWiredCustomData
    {
        ...
        public WiredBoxType Type => WiredBoxType.SelectorUsersWithVariable;
        ...
        private int       referenceUserSource     = 0;
        ...
        public UsersWithVariableBox(Room Instance, Item Item)
        {
        ...
        private static int NormalizeComparison(int v)
        {
            switch (v)
            {
                case CMP_GREATER_THAN: case CMP_GREATER_THAN_OR_EQUAL:
                case CMP_LESS_THAN_OR_EQUAL: case CMP_LESS_THAN: case CMP_NOT_EQUAL:
                    return v;
                default: return CMP_EQUAL;
            }
        }
        ...
        {
            switch (v) { case TARGET_FURNI: case TARGET_CONTEXT: case TARGET_ROOM: return v; default: return TARGET_USER; }
        }

        private static int NormalizeReferenceFurniSource(int v)
        {
            switch (v) { case SOURCE_SECONDARY_SELECTED: case 2: case 3: return v; default: return 0; }
        }

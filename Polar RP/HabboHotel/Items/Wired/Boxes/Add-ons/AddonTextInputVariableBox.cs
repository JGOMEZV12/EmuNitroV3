...
{
    class AddonTextInputVariableBox : IWiredItem, IWiredCustomData
    {
        public Room Instance { get; set; }
        public Item Item { get; set; }
        public WiredBoxType Type => WiredBoxType.AddonTextInputVariable;
        ...
        private string _capturerName = "";

        public AddonTextInputVariableBox(Room instance, Item item)
        {
            Instance = instance;
            Item = item;
        ...
        public void HandleSave(ClientPacket packet)
        {
            ...
            _capturerName = parts.Length > 1 ? parts[1] : "";
            StringData = raw;
        }

        public string GetWiredData() => JsonConvert.SerializeObject(new { variableToken = _variableToken, capturerName = _capturerName });
        public void LoadWiredData(string wiredData) {
            if (string.IsNullOrEmpty(wiredData)) return;
            try {
                var data = JsonConvert.DeserializeObject<dynamic>(wiredData);
                _variableToken = data.variableToken;
                _capturerName = data.capturerName;
            } catch { _variableToken = wiredData; }
            StringData = _variableToken + "\t" + _capturerName;
        }

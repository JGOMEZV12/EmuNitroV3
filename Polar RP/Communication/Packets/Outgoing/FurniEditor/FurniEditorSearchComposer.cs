...
namespace Polar.Communication.Packets.Outgoing.FurniEditor
{
    public class FurniEditorSearchComposer : ServerPacket
    {
     ...
     : base(ServerPacketHeader.FurniEditorSearchComposer)
        {
            // ✅ Orden exacto del parser: count → items → total → page
            ...
            if (items != null)
            {
                foreach (var item in items)
                {
                    ...
                    WriteInteger(GetInt(item, "interaction_modes_count", 1));
                }
            }

            WriteInteger(total);   // ✅ total va AL FINAL
            WriteInteger(page);    // ✅ page va después del total
        }
        // ── Helpers ───────────────────────────────────────────────────────────
